...
async function getResultsFromQuerySql(
  connectionProfile,
  providerText,
  queryText
) {
  let connectionResult = await azdata.connection.connect(
    connectionProfile,
    false,
    false
  );
  let connectionUri = await azdata.connection.getUriForConnection(
...
function getColTypeString(
  ...
  if (
    precisionDataTypes.includes(dataType) ||
    scaleDataTypes.includes(dataType)
  ) {
    typeParts.push("(");
...
function getColTypeAsCSharpString(dataType, isNullable) {
  let typeParts = [];
  let cSharpDT = "";
  switch (dataType) {
    case "nvarchar":
